...
#define WINDOW_SIZE 5    // 滑动窗口大小--已改
#define THRESHOLD 0.1f   // 1/10阈值
...
struct ssd_info *insert2readghostbuffer(struct ssd_info *ssd, unsigned int lpn, unsigned int state)
{
    ...
    if (ssd == NULL || ssd->dram == NULL || ssd->dram->ghostbuffer_read == NULL) {
        return ssd;
    }

    key.group = lpn;
    temp = (struct buffer_group *)hash_find(ssd->dram->ghostbuffer_read, (HASH_NODE *)&key);
    if (temp != NULL) {
        /* 命中：更新访问痕迹 + LRU 提到队头 */
        temp->stored |= state;
        Set_First(ssd->dram->ghostbuffer_read, temp);
        temp->access_Count++;
        return ssd;
    }

    /* 满了：淘汰队尾（安全处理单结点） */
    if (ssd->dram->ghostbuffer_read->current_buffer_page >= ssd->dram->ghostbuffer_read->max_buffer_page) {
        buffer_node = ssd->dram->ghostbuffer_read->buffer_tail;

        if (buffer_node != NULL) {
            if (ssd->dram->ghostbuffer_read->buffer_head == buffer_node &&
                ssd->dram->ghostbuffer_read->buffer_tail == buffer_node) {
                ssd->dram->ghostbuffer_read->buffer_head = NULL;
                ssd->dram->ghostbuffer_read->buffer_tail = NULL;
            } else {
                ssd->dram->ghostbuffer_read->buffer_tail = buffer_node->LRU_link_pre;
                if (ssd->dram->ghostbuffer_read->buffer_tail != NULL) {
                    ssd->dram->ghostbuffer_read->buffer_tail->LRU_link_next = NULL;
                }
                if (ssd->dram->ghostbuffer_read->buffer_head == buffer_node) {
                    ssd->dram->ghostbuffer_read->buffer_head = buffer_node->LRU_link_next;
                    if (ssd->dram->ghostbuffer_read->buffer_head != NULL) {
                        ssd->dram->ghostbuffer_read->buffer_head->LRU_link_pre = NULL;
                    }
                }
            }

            ssd->dram->ghostbuffer_read->current_buffer_page--;
            buffer_node->LRU_link_pre = buffer_node->LRU_link_next = NULL;
            hash_del(ssd->dram->ghostbuffer_read, (HASH_NODE *)buffer_node);
            hash_node_free(ssd->dram->ghostbuffer_read, (HASH_NODE *)buffer_node);
            buffer_node = NULL;
        }
    }
    ...
    new_node->group = lpn;
    new_node->stored = state;      /* 访问形态：本次读触及的 subpage 位图 */
    new_node->dirty_clean = 0;     /* ghost 不存数据、不写回 */
    ...
    if (ssd->dram->ghostbuffer_read->buffer_head == NULL) {
        ssd->dram->ghostbuffer_read->buffer_head = ssd->dram->ghostbuffer_read->buffer_tail = new_node;
    } else {
        new_node->LRU_link_next = ssd->dram->ghostbuffer_read->buffer_head;
        ssd->dram->ghostbuffer_read->buffer_head->LRU_link_pre = new_node;
        ssd->dram->ghostbuffer_read->buffer_head = new_node;
    }

    hash_add(ssd->dram->ghostbuffer_read, (HASH_NODE *)new_node);
    ssd->dram->ghostbuffer_read->current_buffer_page++;
...
/* parityghostbuffer 插入函数：
 * - 只存 key（这里的 lpn 建议传 “trip/stripe id”，也就是你传给 ppc_cache 的第一个参数）
 * - 不存数据
 * - 用 stored |= state 记录“该 trip 被触及的子页位图/范围”（你也可以把 state 当作统计信息）
 * - LRU + hash：命中移到队头；满了淘汰队尾
 */
struct ssd_info *insert2parityghostbuffer(struct ssd_info *ssd, unsigned int lpn, unsigned int state)
{
    struct buffer_group *buffer_node, *new_node, *temp, key;

    /* 防御：ghostbuffer_parity 没初始化 */
    if (ssd == NULL || ssd->dram == NULL || ssd->dram->ghostbuffer_parity == NULL) {
        return ssd;
    }

    key.group = lpn;
    temp = (struct buffer_group *)hash_find(ssd->dram->ghostbuffer_parity, (HASH_NODE *)&key);
    if (temp != NULL) {
        /* 命中：更新元数据 + 提到队头 */
        temp->stored |= state;
        Set_First(ssd->dram->ghostbuffer_parity, temp);
        temp->access_Count++;
        return ssd;
    }

    /* 满了：淘汰队尾（修复“只有一个结点”会崩溃的问题） */
    if (ssd->dram->ghostbuffer_parity->current_buffer_page >= ssd->dram->ghostbuffer_parity->max_buffer_page) {
        buffer_node = ssd->dram->ghostbuffer_parity->buffer_tail;

        if (buffer_node != NULL) {
            if (ssd->dram->ghostbuffer_parity->buffer_head == buffer_node &&
                ssd->dram->ghostbuffer_parity->buffer_tail == buffer_node) {
                /* 只有一个结点 */
                ssd->dram->ghostbuffer_parity->buffer_head = NULL;
                ssd->dram->ghostbuffer_parity->buffer_tail = NULL;
            } else {
                /* 多个结点：摘掉尾结点 */
                ssd->dram->ghostbuffer_parity->buffer_tail = buffer_node->LRU_link_pre;
                if (ssd->dram->ghostbuffer_parity->buffer_tail != NULL) {
                    ssd->dram->ghostbuffer_parity->buffer_tail->LRU_link_next = NULL;
                }

                /* 防御：如果链表异常导致尾结点也是头结点 */
                if (ssd->dram->ghostbuffer_parity->buffer_head == buffer_node) {
                    ssd->dram->ghostbuffer_parity->buffer_head = buffer_node->LRU_link_next;
                    if (ssd->dram->ghostbuffer_parity->buffer_head != NULL) {
                        ssd->dram->ghostbuffer_parity->buffer_head->LRU_link_pre = NULL;
                    }
                }
            }

            ssd->dram->ghostbuffer_parity->current_buffer_page--;
            buffer_node->LRU_link_pre = buffer_node->LRU_link_next = NULL;
            hash_del(ssd->dram->ghostbuffer_parity, (HASH_NODE *)buffer_node);
            hash_node_free(ssd->dram->ghostbuffer_parity, (HASH_NODE *)buffer_node);
            buffer_node = NULL;
        }
    }
    ...
    new_node->group = lpn;          /* 这里的 lpn = trip/stripe id（推荐） */
    new_node->stored = state;       /* 记录累计触及范围（建议传 ppc_cache 里的 state） */
    new_node->dirty_clean = 0;      /* ghost 不参与写回，这里保持 0 即可 */
    new_node->access_Count = 1;
    ...
    if (ssd->dram->ghostbuffer_parity->buffer_head == NULL) {
        ssd->dram->ghostbuffer_parity->buffer_head = ssd->dram->ghostbuffer_parity->buffer_tail = new_node;
    } else {
        new_node->LRU_link_next = ssd->dram->ghostbuffer_parity->buffer_head;
        ssd->dram->ghostbuffer_parity->buffer_head->LRU_link_pre = new_node;
        ssd->dram->ghostbuffer_parity->buffer_head = new_node;
    }

    hash_add(ssd->dram->ghostbuffer_parity, (HASH_NODE *)new_node);
    ssd->dram->ghostbuffer_parity->current_buffer_page++;
...
void reset_access_count(struct buffer_group *head) {
    for (struct buffer_group *p = head; p != NULL; p = p->LRU_link_next) {
        p->access_Count = 1;
    }
}

void decay_access_count(struct buffer_group *head) {
    for (struct buffer_group *p = head; p != NULL; p = p->LRU_link_next) {
        p->access_Count >>= 1;   // 除2
    }
}

int dump_buffer_access_stat(
        struct buffer_group *head,
        const char *name)
{
    long long total = 0;
    int active = 0;
    ...
    printf("[%s] nodes=%d active=%d total_access=%lld\n",
           name, nodes, active, total);
    return nodes;
}
...
void get_statistic(struct ssd_info *ssd, int *write_stats, int *read_stats, int size_in_write, int size_in_read) {
    ...
    int i = 0, j = 0;
    buffer_node = ssd->dram->ghostbuffer_parity->buffer_head;
    databuffer_node = ssd->dram->ghostbuffer_read->buffer_head;
    while (buffer_node != NULL && i < size_in_write) {
        write_stats[i++] = buffer_node->access_Count;

        //test
//        if(buffer_node->access_Count==0){
//            printf("error! write node access cnt 0!!!");
//        }
...
void build_prefix(int *arr_desc, long long *prefix, int len) {
    if (len <= 0) return;
    prefix[0] = arr_desc[0];
    for (int i = 1; i < len; ++i) {
        prefix[i] = prefix[i - 1] + arr_desc[i];
    }
}

static inline long long topk_sum_ll(long long *prefix, int len, int k) {
    if (k <= 0 || len <= 0) return 0;
    if (k >= len) return prefix[len - 1];   // 超过已有节点数：相当于剩下是0
    return prefix[k - 1];
}

int optimize_cache_allocation(struct ssd_info *ssd) {
    //计算两个缓存中各自的节点数量
    int size_in_write = countNodes(ssd->dram->ghostbuffer_parity->buffer_head);
    int size_in_read = countNodes(ssd->dram->ghostbuffer_read->buffer_head);
    int size_all = size_in_write + size_in_read;
    int CAP = ssd->dram->buffer->current_limit + ssd->dram->databuffer->current_limit;

//     1. 初始化统计数组
    int *write_stats = (int *) malloc(size_in_write * sizeof(int));
    if (write_stats == NULL) {
        // 处理内存分配失败的情况，例如返回错误码或进行错误提示
        return -1;
    }
    memset(write_stats, 0, size_in_write * sizeof(int));

    int *read_stats = (int *) malloc(size_in_read * sizeof(int));
    if (read_stats == NULL) {
...
//     3. 降序排序
    ...
    // 4. 计算累加数组
    long long *write_array = (long long *) malloc(size_in_write * sizeof(long long));
    if (write_array == NULL) {
        free(write_stats);
        free(read_stats);
        return -1;
    }
    memset(write_array, 0, size_in_write * sizeof(long long));

    long long *read_array = (long long *) malloc(size_in_read * sizeof(long long));
    if (read_array == NULL) {
        free(write_stats);
        free(read_stats);
        free(write_array);
    ...
    memset(read_array, 0, size_in_read * sizeof(long long));


    /* 前缀和 */
    build_prefix(write_stats, write_array, size_in_write);
    build_prefix(read_stats, read_array, size_in_read);

    /* ==== DEBUG: window statistics sanity check ==== */
    long long write_total = (size_in_write > 0) ? write_array[size_in_write - 1] : 0;
    long long read_total  = (size_in_read  > 0) ? read_array[size_in_read  - 1] : 0;

    int write_max = (size_in_write > 0) ? write_stats[0] : -1;
    int read_max  = (size_in_read  > 0) ? read_stats[0]  : -1;

    printf("[WINSTAT] W_nodes=%d R_nodes=%d CAP=%d\n",
           size_in_write, size_in_read, CAP);

    printf("[WINSTAT] write_max=%d write_total=%lld\n",
           write_max, write_total);

    printf("[WINSTAT] read_max=%d read_total=%lld\n",
           read_max, read_total);
/* =============================================== */
    if (write_total + read_total == 0) {
        // 没有任何统计信号，返回当前写配额，避免 best_j 退化到 0/1
        return ssd->dram->buffer->current_limit;
    }
    ...
    /* 枚举写容量 j=0..CAP，读容量=CAP-j */
    int best_j = 0;
    long long best_profit = -1;

    for (int j = 0; j <= CAP; ++j) {//如果是size_all,那 best_j 的语义就会变成“在当前节点集合里分配份额”，而你后面拿它当“目标写容量”会出现偏差。
        int write_cap = j;
        int read_cap  = CAP - j;

        long long sw = topk_sum_ll(write_array, size_in_write, write_cap); // cap超过节点数 => 自动饱和
        long long sr = topk_sum_ll(read_array, size_in_read, read_cap);

        long long profit = sw * 10  + sr * 1;
...
//void Evict_one_node(struct ssd_info *ssd, struct buffer_info_Hash *buffer) {//弹一个结点腾空间
//    struct buffer_group *buffer_node;
//    // 定义目标队列：根据输入队列确定要迁移到的另一个队列
//    struct buffer_info_Hash *target_buffer = NULL;
//
//    // 1. 确定输入队列和目标队列的对应关系
//    if (buffer == ssd->dram->databuffer) {   // 输入是读缓冲区 → 迁移到写缓冲区
//        target_buffer = ssd->dram->buffer;
//    } else if (buffer == ssd->dram->buffer) {  // 输入是写缓冲区 → 迁移到读缓冲区
//        target_buffer = ssd->dram->databuffer;
//    } else {
//        printf("Error: Invalid buffer pointer in Evict_one_node\n");
//        return;
//    }
//
//    // 2. 从输入队列的队尾取出节点（核心：淘汰队尾节点）
//    buffer_node = buffer->buffer_tail;
//    if (!buffer_node) {
//        return;  // 防御性检查：输入队列空，直接返回
//    }
//
//    // 3. 从输入队列中剔除该节点（更新链表指针）
//    // 3.1 更新前驱节点的next指针
//    if (buffer_node->LRU_link_pre) {
//        buffer_node->LRU_link_pre->LRU_link_next = NULL;
//    } else {
...
//    // 3.3 从输入队列的哈希表中删除（保留hash_del，仅移除映射，不释放节点）
//    hash_del(buffer, (HASH_NODE *) buffer_node);
//
//    // 4. 重置节点的链表指针（避免携带原队列的指针）
//    buffer_node->LRU_link_pre = NULL;
//    buffer_node->LRU_link_next = NULL;
//
//    // 5. 将节点插入到目标队列的尾部（核心修改：插入尾部而非头部）
//    if (target_buffer->buffer_tail == NULL) {
//        // 目标队列为空：头尾都指向该节点
//        target_buffer->buffer_head = buffer_node;
//        target_buffer->buffer_tail = buffer_node;
//    } else {
//        // 目标队列非空：插入到尾部
//        buffer_node->LRU_link_pre = target_buffer->buffer_tail; // 新节点的前驱指向原尾部
//        target_buffer->buffer_tail->LRU_link_next = buffer_node; // 原尾部的后继指向新节点
//        target_buffer->buffer_tail = buffer_node; // 更新目标队列的尾指针为新节点
//    }
//
//    // 6. 更新队列计数和limit（核心：原队列减，目标队列加）
//    // 6.1 缓冲区页面计数
//    buffer->current_buffer_page--;               // 输入队列计数-1
//    target_buffer->current_buffer_page++;         // 目标队列计数+1
//    // 6.2 缓冲区限额调整（保持总限额不变）
//    if (buffer->current_limit > 0) {
//        buffer->current_limit--;                  // 输入队列限额-1
//        target_buffer->current_limit++;           // 目标队列限额+1
//    }
//
//    // 7. 将节点加入目标队列的哈希表（恢复哈希映射）
//    hash_add(target_buffer, (HASH_NODE *) buffer_node);
//
//    // 清空指针（防御性编程）
//    buffer_node = NULL;
//}
...
void raid_init(struct ssd_info *ssd) {
    unsigned long long pageNum = ((ssd->parameter->page_block) * (ssd->parameter->block_plane) *
                                  (ssd->parameter->plane_die) * (ssd->parameter->die_chip) *
                                  (ssd->parameter->chip_channel[0]) * (ssd->parameter->channel_number));
    ...
    ssd->stripe.now = 0;
    ssd->stripe.check = 0;
    ssd->stripe.checkLpn = ((ssd->parameter->subpage_page * ssd->parameter->page_block * ssd->parameter->block_plane *
...
void add_invalid_date(struct ssd_info *ssd) {
    ...
    ssd->allPage = ((ssd->parameter->page_block) * (ssd->parameter->block_plane) * (ssd->parameter->plane_die) *
                    (ssd->parameter->die_chip) * (ssd->parameter->chip_channel[0]) *
                    (ssd->parameter->channel_number * 3 / 4));
...
void add_valid_date(struct ssd_info *ssd) {
    ...
    ssd->allPage = ((ssd->parameter->page_block) * (ssd->parameter->block_plane) * (ssd->parameter->plane_die) *
                    (ssd->parameter->die_chip) * (ssd->parameter->chip_channel[0]) * (ssd->parameter->channel_number));
            ...
            if (++ssd->stripe.now == (ssd->stripe.all - 1)) {
                ...
                for (i = 0, j = 0; i < ssd->stripe.all; ++i) {
                    if (i == ssd->stripe.check) {
                        raid_pre_read(ssd, ssd->stripe.checkLpn * ssd->parameter->subpage_page, ssd->stripe.checkLpn,
                                      mask, full_page);
                        continue;
                    }
...
struct sub_request *
get_sub_request_for_recovery_2(struct ssd_info *ssd, int channel, int chip, int die, int plane, int block, int page) {
    struct sub_request *sub = (struct sub_request *) malloc(
            sizeof(struct sub_request));                        /*����һ��������Ľ�???*/
...
struct sub_request *
create_recovery_sub_write_2(struct ssd_info *ssd, int channel, int chip, int die, int plane, int block, int page) {
    unsigned int mask = 0;
    struct sub_request *sub = get_sub_request_for_recovery_2(ssd, channel, chip, die, plane, block, page);
...
struct ssd_info *simulate_for_recovery(struct ssd_info *ssd) {
    ...
    while (1) {
        ...
        if (ssd->cpu_current_state == CPU_IDLE ||
            (ssd->cpu_next_state == CPU_IDLE && ssd->cpu_next_state_predict_time >= ssd->current_time)) {
    ...
    while (get_req_num(ssd) > 0) {
        update_time(ssd);
        if (++i % 1000 == 0) {
            printf("i is %lld all %lld time %lld  read %lld\n", i, get_req_num(ssd), ssd->current_time,
                   get_req_num_read(ssd));
        }
...
int main(int argc, char *argv[]) {
            ...
            for (int die = 0; die < ssd->parameter->die_chip; die++) {
                for (int plane = 0; plane < ssd->parameter->plane_die; plane++) {
                    for (int block = 0; block < ssd->parameter->block_plane; block++) {
                        if (ssd->channel_head[chan].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].free_page_num !=
    ...
    if (ssd->parameter->fast_gc == 1) {
        printf("Fast GC is activated, %.2f:%.2f and %.2f:%.2f.\n", ssd->parameter->fast_gc_thr_1,
               ssd->parameter->fast_gc_filter_1, ssd->parameter->fast_gc_thr_2, ssd->parameter->fast_gc_filter_2);
    }
...
struct ssd_info *simulate_multiple(struct ssd_info *ssd, int sTIMES) {
    ...
    fprintf(ssd->statisticfile,
            "requests            time       gc_count          r_lat          w_lat          w_lsb          w_csb          w_msb           f_gc       mov_page      free_lsb     r_req_count    w_req_count    same_pages       req_lsb       req_csb       req_msb       w_req_count_l    same_pages_l       req_lsb_l       req_csb_l       req_msb_l\n");
    ...
    while (simulate_times < sTIMES) {
        ...
        while (flag != 100) {
            ...
            if (flag == 1) {
                //printf("once\n");
                if (0 && ssd->parameter->dram_capacity != 0) {
                    buffer_management(ssd);
                    //distribute(ssd);
                } else {
                    no_buffer_distribute(ssd);
	...
	***********************************************************************************************************/
    if (ssd->parameter->allocation_scheme == 0 && ssd->parameter->dynamic_allocation == 2)
        large_lsn = (int) ((ssd->parameter->subpage_page * ssd->parameter->page_block * ssd->parameter->block_plane *
                            ssd->parameter->plane_die * ssd->parameter->die_chip * (ssd->parameter->chip_num)) *
                           (1 - ssd->parameter->overprovide) * 0.75);
    else
        large_lsn = (int) ((ssd->parameter->subpage_page * ssd->parameter->page_block * ssd->parameter->block_plane *
                            ssd->parameter->plane_die * ssd->parameter->die_chip * ssd->parameter->chip_num) *
                    ...
                    for (j = 0; j < 32 / ssd->parameter->subpage_page; j++) {
                        ...
                        if (k != 0) {
                            ...
                            lpn = start / ssd->parameter->subpage_page +
                                  ((end - start - 1) / 32 - i) * 32 / ssd->parameter->subpage_page + j;
...
void delete_req(struct ssd_info *ssd, struct request **pre_node, struct request **req) {
    ...
    while ((*req)->subs != NULL) {
        ...
        unsigned long long READTIME =
                ...
                (tmp->size * ssd->parameter->subpage_capacity) * ssd->parameter->time_characteristics.tRC;
        if (tmp->operation == READ) {
            ssd->chanSubRWriteTime[tmp->location->channel] += ((tmp->complete_time - (*req)->time) - READTIME);
            ++ssd->subRWaitCount;
        } else {
            unsigned long long WRITETIME = 7 * ssd->parameter->time_characteristics.tWC +
                                           (tmp->size * ssd->parameter->subpage_capacity) *
        ...
        if (tmp->update != NULL) {
            ...
            ssd->chanSubRWriteTime[tmp->update->location->channel] += ((tmp->update->complete_time - (*req)->time) -
                                                                       READTIME);
            ++ssd->subRWaitCount;
            //}
...
void trace_output(struct ssd_info *ssd) {
    ...
    while (req != NULL) {
        ...
        start_time = 0;
        end_time = 0;
        if (req->response_time != 0)//请求是否已标记完成---若响应时间已设置，说明已标记为完成
        {
            ...
            window_opt_hook(ssd);//优化逻辑111

            /* ---- window-based control: every 10K requests ---- */
//            if (ssd->completed_request_count > 0 &&
//                ssd->completed_request_count % 10240 == 0 &&
//                ssd->completed_request_count != ssd->last_opt_completed) {
//
//                ssd->last_opt_completed = ssd->completed_request_count; //哨兵--保证每个边界只执行一次
//
//                printf("\n=== WINDOW @ %lu ===\n", ssd->completed_request_count);
//
//                dump_buffer_access_stat(
//                        ssd->dram->buffer->buffer_head,
//                        "PARITY");
//
//                dump_buffer_access_stat(
//                        ssd->dram->databuffer->buffer_head,
//                        "READ");
//
//                /* 1) 计算目标写容量 target_w (= best_j) */
//                int target_w = optimize_cache_allocation(ssd);
...
//                printf("cur_w:%d cur_r:%d CAP:%d\n", cur_w, cur_r, CAP);
//
//                /* 3) 最小配额（你原来用 1/4，这里沿用；可按需调整） */
//                int MIN_W = CAP / 10;
//                int MIN_R = CAP / 10;
//
//                /* 4) 迁移速率：每窗口最多迁移 MAX_STEP 页
//                      CAP=8064 时，1%≈80页/窗口；代价不大可取 80~160 */
//                int MAX_STEP = CAP / 50;   /* ~160 */
//                if (MAX_STEP < 16) MAX_STEP = 16;
//
//                /* 5) 迟滞：目标变化不大不动，避免抖动（可选但推荐） */
//                int H = 64; /* 或 CAP/100 (~80) */
//                int diff = target_w - cur_w;
//                int absdiff = (diff >= 0) ? diff : -diff;
//                printf("diff target-cur_w = %d\n", target_w - cur_w);
//
...
//            }
            ...
            //输出端到端延迟到指定文件--已改
            long long latency = (long long) (req->response_time - req->time);
            if (latency < 0) latency = 0;
            fprintf(ssd->latencyfile, "%lld\n", latency);
            ...
            if (req->operation == READ)//更新全局的延迟统计
            {
        ...
        {
            //printf("Rsponse time = 0!\n");
            if (ssd->parameter->dram_capacity != 0) {//判断是否有缓存
                while (sub != NULL && req->operation == READ) {//---这里判断req的类型，而不是判断sub类型，因为
                    if (((sub->current_state == SR_COMPLETE) ||
                         ((sub->next_state == SR_COMPLETE) && (sub->next_state_predict_time <= ssd->current_time))) &&
                        sub->state != 0)    // if any sub-request is not completed, the request is not completed
                    {
                        if (ssd->dram->map->map_entry[sub->lpn].flag == -2) {//插入到两个缓存
                            ssd = insert2buffer(ssd, sub->lpn, sub, req);
                            ssd = insert2ghost(ssd, sub->lpn, sub, req);
                            ssd->dram->map->map_entry[sub->lpn].flag = 1;
                        }else if (ssd->dram->map->map_entry[sub->lpn].flag == -1) {//插入到ghostbuffer
                            ssd = insert2ghost(ssd, sub->lpn, sub, req);
                            ssd->dram->map->map_entry[sub->lpn].flag = 1;
                        }else if (ssd->dram->map->map_entry[sub->lpn].flag == 0) { //表示需插入到普通缓存
                            ssd = insert2buffer(ssd, sub->lpn, sub, req);
                            ssd->dram->map->map_entry[sub->lpn].flag = 1;
                        }
            ...
            if (flag == 1 && req->completeFlag == 0) {
                ...
                fprintf(ssd->outputfile, "%16lld %10d %6d %2d %16lld %16lld %10lld", req->time, req->lsn, req->size,
                        req->operation, start_time, end_time, end_time - req->time);
                fprintf(ssd->outputfile, " %lu %u %u %u", ssd->direct_erase_count + ssd->normal_erase, 0, 0,
                        ssd->moved_page_count);
                fprintf(ssd->outputfile, " %u\n", req->operation);
                fflush(ssd->outputfile);
                ssd->completed_request_count++;

                window_opt_hook(ssd);//优化逻辑222
//                if (ssd->completed_request_count > 0 &&
//                    ssd->completed_request_count % 10240 == 0 &&
//                    ssd->completed_request_count != ssd->last_opt_completed) {
//
...
//                }
                ...
                long long latency = (long long)(end_time - req->time);
                if (latency < 0) latency = 0;
                fprintf(ssd->latencyfile, "%lld\n", latency);
                fflush(ssd->latencyfile);
                ...
                //对10001个请求做总结
                if (ssd->completed_request_count % 10000 == 0) {
                    printf("completed requests: %ld, max_queue_depth: %d, ", ssd->completed_request_count,
                           ssd->max_queue_depth);
                    printf("free_lsb: %ld, free_csb: %ld, free_msb: %ld\n", ssd->free_lsb_count, ssd->free_csb_count,
                           ssd->free_msb_count);
...
void statistic_output(struct ssd_info *ssd) {
                ...
                for (p = 0; p < ssd->parameter->plane_die; p++) {
                    ...
                    for (m = 0; m < ssd->parameter->block_plane; m++) {
                        if (ssd->channel_head[i].chip_head[j].die_head[k].plane_head[p].blk_head[m].erase_count > 0) {
                            erase = erase +
                                    ssd->channel_head[i].chip_head[j].die_head[k].plane_head[p].blk_head[m].erase_count;
                    ...
                    fprintf(ssd->outputfile, "the %d channel, %d chip, %d die, %d plane has : %13d erase operations\n",
                            i, j, k, p, plane_erase);
                    fprintf(ssd->statisticfile,
                            "the %d channel, %d chip, %d die, %d plane has : %13d erase operations\n", i, j, k, p,
                            plane_erase);
                }
            }
        }
    ...
    for (i = 0; i < ssd->parameter->channel_number; i++) {
        ...
        fprintf(ssd->statisticfile, "%f\t", available_capacity_channel /
                                            (ssd->parameter->chip_channel[0] * ssd->parameter->die_chip *
                                             ssd->parameter->plane_die * ssd->parameter->block_plane *
                                             ssd->parameter->page_block));
    }
    fprintf(ssd->statisticfile, "\n");
    printf("available capacity:%f\n", available_capacity /
                                      (ssd->parameter->channel_number * ssd->parameter->chip_channel[0] *
    ...
    for (i = 0; i < ssd->parameter->channel_number; ++i) {
        fprintf(ssd->statisticfile, "%f\t",
                ((double) (ssd->channel_head[i].blockCountR + ssd->channel_head[i].blockCountW)) /
                ((double) (ssd->channel_head[i].completeCountR + ssd->channel_head[i].completeCountW)));
    }
    fprintf(ssd->statisticfile, "\n");
    fprintf(ssd->statisticfile, "channel count 2\n");
    for (i = 0; i < ssd->parameter->channel_number; ++i) {
        fprintf(ssd->statisticfile, "%lld\t",
                ssd->channel_head[i].completeCountW + ssd->channel_head[i].completeCountR);;
    }
    fprintf(ssd->statisticfile, "\n");
...
void statistic_output_easy(struct ssd_info *ssd, unsigned long completed_requests_num) {
    ...
    fprintf(ssd->statisticfile, "%ld, %16lld, %13ld, %13ld, %13ld, %13ld, %13ld, %13ld, ", completed_requests_num,
            ssd->current_time, ssd->erase_count, read_avg_lat, write_avg_lat, ssd->newest_write_lsb_count,
            ssd->newest_write_csb_count, ssd->newest_write_msb_count);
    fprintf(ssd->statisticfile, "%13ld, %13d, %13ld, %13d, %13d, ", ssd->fast_gc_count, ssd->moved_page_count,
            ssd->free_lsb_count, ssd->newest_read_request_count, ssd->newest_write_request_count);
    fprintf(ssd->statisticfile, "%13d, %13d, %13d, %13d, ", ssd->newest_write_request_completed_with_same_type_pages,
            ssd->newest_req_with_lsb, ssd->newest_req_with_csb, ssd->newest_req_with_msb);
    fprintf(ssd->statisticfile, "% 13d, %13d, %13d, %13d, %13d, %13ld\n", ssd->newest_write_request_num_l,
            ssd->newest_write_request_completed_with_same_type_pages_l, ssd->newest_req_with_lsb_l,
            ssd->newest_req_with_csb_l, ssd->newest_req_with_msb_l, write_avg_lat_l);
...
void free_all_node(struct ssd_info *ssd) {
    ...
    unsigned long long StripeNum =
            ssd->parameter->page_block * ssd->parameter->block_plane * ssd->parameter->plane_die *
    ...
    hash_destroy(ssd->dram->databuffer);
    ssd->dram->databuffer = NULL;

    hash_destroy(ssd->dram->ghostbuffer);
    ssd->dram->ghostbuffer = NULL;

    hash_destroy(ssd->dram->ghostbuffer_parity);
    ssd->dram->ghostbuffer_parity = NULL;

    hash_destroy(ssd->dram->ghostbuffer_read);
    ssd->dram->ghostbuffer_read = NULL;
        ...
        for (cp_id = 0; cp_id < ssd->parameter->chip_channel[0]; cp_id++) {
            ...
            for (di_id = 0; di_id < ssd->parameter->die_chip; di_id++) {
                ...
                for (pl_id = 0; pl_id < ssd->parameter->plane_die; pl_id++) {
                    ...
                    lpn = 0;
                    while (ssd->channel_head[cn_id].chip_head[cp_id].die_head[di_id].plane_head[pl_id].free_page >
                           (ssd->parameter->page_block * ssd->parameter->block_plane) * 0.3) {
...
*********************************************************************************************/
struct sub_request *
get_sub_request_for_recovery(struct ssd_info *ssd, int channel, int chip, int die, int plane, int block, int page,
                             struct request *req) {
        ...
        for (i = 0; i < ssd->stripe.all - 1; ++i) {
            ...
            if (ssd->dram->map->map_entry[lpn].state != 0 && ssd->dram->map->map_entry[lpn].oldppa == -1) {
                ...
                if (1) {
                    ...
                    if (ssd->channel_head[location->channel].subs_r_tail !=
                        NULL)            /*�����µĶ����󣬲��ҹҵ�channel��subs_r_tail����β*/
                    {
...
void gc_eject(struct ssd_info *ssd, unsigned long long raidid) {
    ...
    if ((buffer_node = (struct buffer_group *) hash_find(ssd->dram->buffer, (HASH_NODE *) &key)) != NULL) {
        ...
        sub = creat_sub_request(ssd, ssd->stripe.checkLpn, sub_req_size, sub_req_state, ssd->raidReq, WRITE, 0,
                                sub_req_lpn);
        ...
        ssd->dram->buffer->current_buffer_page--;//已改
        hash_del(ssd->dram->buffer, (HASH_NODE *) buffer_node);
        hash_node_free(ssd->dram->buffer, (HASH_NODE *) buffer_node);
        ssd->parityChange++;
        if (sub_req_lpn == 9154) {
...
void movtiavate(struct ssd_info *ssd, struct buffer_group *buffer_node) {
    ...
    for (int i = 1; i < ssd->stripe.all - 2; ++i) {
        if (buffer_node->accessTime[index[diff]] - buffer_node->accessTime[index[diff + 1]] <
            buffer_node->accessTime[index[i]] - buffer_node->accessTime[index[i + 1]]) {
            diff = i;
        }
    }
...
void ppc_cache(struct ssd_info *ssd, int lpn, unsigned int state, struct request *req, unsigned int mask,
               unsigned long long userlpn, struct sub_request *presub) {
    struct buffer_group *buffer_node = 0, key;
    int i;
...
//            if (ssd->aCountParityfile) {
//                log_to_csv(ssd->aCountParityfile, buffer_node->group,buffer_node->access_Count);//
//            } else {
//                printf("Failed to open aCountParityfile.\n");
//            }

            buffer_node->access_Count = 0;
            ssd->dram->buffer->current_buffer_page--;
            hash_del(ssd->dram->buffer, (HASH_NODE *) buffer_node);
            hash_node_free(ssd->dram->buffer, (HASH_NODE *) buffer_node);
...
//        }
        ...
        buffer_node->accessCount = malloc(sizeof(unsigned int) * ssd->stripe.all);//为什么申请4个int空间--用一个数组存node的访问次数
        ...
        if (req && ssd->trip2Page[buffer_node->group].location) {
            creat_sub_request(ssd, ssd->stripe.checkLpn, size(ssd->dram->map->map_entry[userlpn].state),
                              ssd->dram->map->map_entry[userlpn].state, req, READ, 0, buffer_node->group);

            //printf("ssd->stripe.checkLpn:%u\n",ssd->stripe.checkLpn);---8388609
        }
    }
    ...
    if (ssd->dram->map->map_entry[userlpn].state != 0 && req && presub && presub->update == NULL) {
        creat_sub_request(ssd, userlpn, size(ssd->dram->map->map_entry[userlpn].state),
                          ssd->dram->map->map_entry[userlpn].state, req, READ, 0, ssd->page2Trip[userlpn]);
        //printf("userlpn:%u\n",userlpn);-----没进这个if
    }
...
void creat_sub_write_request_for_raid(struct ssd_info *ssd, int lpn, unsigned int state, struct request *req,
                                      unsigned int mask) {
    unsigned int i, j;
    ...
    if (ssd->stripe.now == 0) {
        ...
        ssd->trip2Page[ssd->stripe.nowStripe].Pchannel = ssd->channelToken;//已改
        if (ssd->trip2Page[ssd->stripe.nowStripe].Pchannel >= 16) {
            printf("%llu %d %d\n", ssd->ssdToken, ssd->channelToken, ssd->stripe.check);
            abort();
        }
        ssd->paritycount[ssd->trip2Page[ssd->stripe.nowStripe].Pchannel]++;
        if ((((ssd->trip2Page[ssd->stripe.nowStripe].Pchannel / (ssd->stripe.all * ssd->perChanSSD)) + 1) *
             (ssd->stripe.all * ssd->perChanSSD) == ssd->parameter->channel_number) &&
            (ssd->channelToken == (ssd->perChanSSD - 1))) {
            if (++ssd->stripe.check >= (ssd->stripe.all))
                ssd->stripe.check = 0;
        }
    }
            ...
            for (int i = 0; i < ssd->stripe.all - 1; ++i) {
                ...
                if (ssd->raidBitMap[pos] & (1 << offset)) {
                    if (ssd->dram->map->map_entry[lpn].state)
                        sub = creat_sub_request(ssd, lpn, size(ssd->dram->map->map_entry[lpn].state),
                                                ssd->dram->map->map_entry[lpn].state, \
                        req, READ, 0, raidID);
...
//                if (ssd->target_valid && ssd->completed_request_count >= 10240 &&
//                    ssd->dram->databuffer->current_buffer_page >= ssd->dram->databuffer->current_limit &&
//                    ssd->max_profit_index < ssd->dram->buffer->current_limit &&
//                    ssd->dram->buffer->current_limit > (ssd->dram->buffer->max_buffer_page) / 4) {
...
//			sub=creat_sub_request(ssd,lpn,sub_size,sub_state,req,req->operation,0, ssd->page2Trip[lpn]);
        ...
        while (lpn <= last_lpn) {
            ...
            if (ssd->dram->map->map_entry[lpn].state == 0) {
//                if (ssd->target_valid && ssd->completed_request_count > 10240 &&
//                    ssd->dram->buffer->current_buffer_page >= ssd->dram->buffer->current_limit &&
//                    ssd->max_profit_index > ssd->dram->buffer->current_limit &&
//                    ssd->dram->databuffer->current_limit > (ssd->dram->databuffer->max_buffer_page) / 4) {
//                    Evict_one_node(ssd, ssd->dram->databuffer);      //给写分
//                    //ssd->max_profit_index--;
//                }
...
//        int current_profit = write_array[j] * 5 +
//                             read_array[size_all - j - 1];
//        int max_profit = write_array[max_profit_index] * 5 +
...
int compare_cache_sliding(struct ssd_info *ssd) {
    ...
    int flag = 0;
    if (write_avg / read_avg < THRESHOLD &&
        ssd->dram->databuffer->current_buffer_page == ssd->dram->databuffer->current_limit) {
        flag = 1;
