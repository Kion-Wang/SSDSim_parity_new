...
struct ssd_info{
    ...
    unsigned long long readHit;
    unsigned long long readNotHit;
...
struct chip_info{
	...
	int64_t current_time;               //记录该chip的当前时间//开始这个状态的起始时间
	int64_t next_state_predict_time;    //the predict time of next state, used to decide the sate at the moment
...
struct blk_info{
	...
	int free_page_num;        //记录该块中的free页个数，同上
	int invalid_page_num;     //记录该块中失效页的个数，同上
...
typedef struct buffer_group{//缓存的节点大小是lpn，里面的成员group是该节点的第一个lsn
	HASH_NODE node;                     //哈希节点的结构一定要放在用户自定义结构的最前面，注意!
	struct buffer_group *LRU_link_next;	// next node in LRU list
	struct buffer_group *LRU_link_pre;	// previous node in LRU list

	unsigned int group;                 //（该节点对应的lpn的起始lsn）the first data logic sector number of a group stored in buffer
	...
	unsigned int read_count;
    unsigned int access_Count;          //新建--节点访问次数
...
struct sub_request{
	...
	unsigned int operation;            //表示该子请求的类型，除了读1 写0，还有擦除erase，two plane等操作
	int size;
