			...
			for (k = 0; k < ssd->parameter->die_chip; k++)
			{
				for (m = 0; m < ssd->parameter->plane_die; m++)
				{
					for(n = 0; n < ssd->parameter->block_plane; n++){
						ssd->channel_head[i].chip_head[j].die_head[k].plane_head[m].blk_head[n].erase_count = 0;	
						ssd->channel_head[i].chip_head[j].die_head[k].plane_head[m].blk_head[n].read_count = 0;
					}
...
*/
	...
	ssd->motivateFile=fopen("motivateFile","w");
	if(ssd->motivateFile==NULL)
	{
		printf("the output file can't open\n");
		return NULL;
	}
...
//    unsigned int parity_capacity=dram->dram_capacity;
    ...
    dram->databuffer->buffer_head=dram->databuffer->buffer_tail=NULL;

    dram->ghostbuffer=(tHash*) hash_create((void*) freeFunc);
    dram->ghostbuffer->max_buffer_page=ghost_capacity/3;
    dram->ghostbuffer->current_buffer_page=0;
    dram->ghostbuffer->read_hit=dram->ghostbuffer->read_miss_hit=0;//已改---数据缓存读命中初始化
	...
	printf("page_num is %d %d\n",page_num, ssd->parameter->chip_num);

	for(i = 0; i < page_num; ++i){
		dram->map->map_entry[i].oldppa = -1;
	}
