        ...
        while (lpn <= last_lpn)//当前lpn小于或等于最大目标页号last_lpn时，也就是说明该IO读请求操作尚未完成
        {
            ...
            if (buffer_node == NULL) {
                ...
                if ((buffer_node->stored & offset) == offset)//读-全命中---提到head
                {
                    ...
                    Set_First(ssd->dram->databuffer, buffer_node);
                } else//---部分命中
                {
                   // ssd->dram->map->map_entry[lpn].lpnFlag = 1;
...
int find_node_READ(struct ssd_info *ssd, unsigned int lpn, struct request *req) {
    unsigned int offset;
    struct buffer_group *buffer_node_d, key;
    int result;
    offset = get_offset(ssd, req, lpn);
    key.group = lpn;
    buffer_node_d = (struct buffer_group *) hash_find(ssd->dram->databuffer, (HASH_NODE *) &key);
    if (buffer_node_d == NULL) {
        ssd->readNotHit++;
        result = find_node_ghost(ssd,lpn,req);
        return result;
    }else{
        if((buffer_node_d->stored & offset) == offset)//读-全命中---提到head
        {
            ...
            ssd->dram->map->map_entry[lpn].flag = 1;
            return 1;//表示命中
        }else//---部分命中---也不算命中
        {
            ssd->readNotHit++;
...
int find_node_ghost(struct ssd_info *ssd, unsigned int lpn, struct request *req) {
    unsigned int offset;
    struct buffer_group *buffer_node, key;
    ...
    if (buffer_node == NULL) {
        if (ssd->dram->databuffer->current_buffer_page < ssd->dram->databuffer->max_buffer_page){//最开始读缓存未满，需利用起来
            ...
            ssd->dram->map->map_entry[lpn].flag = -1;//只插入到ghost缓存
            return -1;
        }
    } else {
...
struct ssd_info *insert2buffer(struct ssd_info *ssd, unsigned int lpn, struct sub_request *sub,
                               struct request *req)//参数sub的作用是针对读请求往缓存写--部分命中的情况
{
    ...
    if (ssd->dram->databuffer->current_buffer_page >= ssd->dram->databuffer->max_buffer_page) {
        ...
        if (buffer_node->dirty_clean > 0) {//脏的才弹，干净的不弹--直接删除结点
            ...
            if (req->operation == READ) {//已改---NULL->req
                creat_sub_request(ssd, buffer_node->group, size(buffer_node->stored), buffer_node->stored, req, WRITE,
                                  0, ssd->page2Trip[sub_req_lpn]);//将尾结点这个请求挂到请求队列，结点弹下去后，新读结点才能进来，方便计算时间
            } else {
...
//        }
        ...
        ssd->dram->ghostbuffer->buffer_tail = ssd->dram->ghostbuffer->buffer_tail->LRU_link_pre;
...
void Set_First(struct buffer_info_Hash *buffer, struct buffer_group *buffer_node) {
